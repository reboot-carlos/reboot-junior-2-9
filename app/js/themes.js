...
function applyColorTheme(theme) {
  ...
  // Alias pour compatibilité avec styles existants
  root.style.setProperty('--rouge-clair',  theme.secondary);
  root.style.setProperty('--rose-pastel',  theme.accent);

  // Mise à jour dynamique de la zone de chat (surcharge l'animation neon-glow)
  ...
  if (typeof saveSetting === 'function') {
    saveSetting('theme_primary', theme.primary);
  }
...
function initThemes() {
  ...
  if (typeof getSetting === 'function') {
    const savedPrimary = getSetting('theme_primary', '');
    if (savedPrimary) {
      const saved = THEMES.find(t => t.primary === savedPrimary);
